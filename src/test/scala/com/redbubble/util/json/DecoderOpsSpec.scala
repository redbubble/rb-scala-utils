...
import com.redbubble.util.json.DecoderOps.decodeFocusNullableAsOption
...
final class DecoderOpsSpec extends Specification with SpecHelper {
  ...
  "Null fields" >> {
    ...
    val fromStringDecoder = Decoder.instance { c =>
      c.downField("some_number").as[Option[String]](decodeFocusNullableAsOption)
    }

    "can be decoded as a None" >> {
      val decoded = decode(containsNullFields)(fromStringDecoder)
      decoded must beRight(None)
    }
  }
